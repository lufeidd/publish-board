                    ...
                    <table v-if="type == 3" style="table-layout: fixed;">
                      ...
                      <tbody v-if="list.length > 0">
                        <tr v-for="(item, index) in list" :key="index">
                          ...
                          <td style="text-align: right">{{item.onsale_num}}</td>
                    ...
                    <table style="table-layout: fixed;" v-else>
                      ...
                      <thead>
                        <tr>
                          ...
                          <td style="text-align: right" v-if="type == 2">上市时长</td>
                      ...
                      <tbody v-if="list.length > 0">
                        <tr v-for="(item, index) in list" :key="index">
                          ...
                          <td style="text-align: right">
                            <span v-if="type == 2">{{item.publish_duration}}个月</span>
...
import {
  INDUSTRY_RANK_GOODS,
  INDUSTRY_RANK_GOODSINCR,
  INDUSTRY_RANK_GOODSNEW,
  INDUSTRY_RANK_SUPPLIER,
...
export default {
  ...
  methods: {
    async getData() {
      ...
      let res;
      if (this.type == 0) {
        res = await INDUSTRY_RANK_GOODS(data);
      } else if (this.type == 1) {
        res = await INDUSTRY_RANK_GOODSINCR(data);
      } else if (this.type == 2) {
        res = await INDUSTRY_RANK_GOODSNEW(data);
      } else if (this.type == 3) {
        res = await INDUSTRY_RANK_SUPPLIER(data);
      }
      if (res.code == 0) {
        this.pagePower = true;
