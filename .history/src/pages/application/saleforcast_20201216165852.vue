            ...
            <div class="model-container">
              <div class="model-bg">
                ...
                <div style="padding: 15px">
                  <div>
                    <span style="margin-right: 5px;font-size:12px;">销售周期</span>
                    <a-cascader
                      :options="dayList"
                      :allowClear="false"
                      v-model="chooseDay"
                  ...
                  <div style="margin-top: 10px">
                    <span style="margin-right: 5px;font-size:12px;">事件影响</span>
                    <a-select
                      mode="multiple"
                      v-model="influence"
                      placeholder="选择事件影响"
...
import { FORECAST_DMS,FORECAST_GOODS } from "../../apis/application.js";
export default {
  data() {
    return {
      pagePower: true,
  ...
  methods: {
    async getPower() {
      ...
      let res = await FORECAST_GOODS(data);
      if (res.code == 0) {
        this.pagePower = true;
        ...
        setTimeout(() => {
          ...
          _this.goodsList = _this.goodsList.map((value, key) => {
            if (value.goods_id == res.data[key].goods_id) {
              let _result = Math.round(
                res.data[key].goods_dms * _nums * _this.chooseDay[0]
              );
              // let _result = 1220;
              if (_result < 10) {
                _result = "少于10册";
                ...
                _result = "100~200册";
              } else if (200 < _result && _result < 1000) {
                let _num = Math.floor(_result / 100) * 100;
                _result = (_num - 50) + "~" + (_num + 50) + "册";
              } else if (1000 <= _result) {
                let _num = Math.floor(_result / 100) * 100;
                _result = (_num - 100) + "~" + (_num + 100) + "册";
              }
    ...
    selected(item1, index1) {
      ...
      if (_check) {
        ...
        this.popupGoodsList.unshift(_obj);
    ...
    addhandle() {
      this.popupGoodsList.map((value, index) => {
        let _obj = {},_coun = 0;
        _obj.cover_pic = value.cover_pic;
        _obj.goods_id = value.goods_id;
        _obj.goods_name = value.title;
        _obj.dms = 0;
        _obj.day = 30;
        this.goodsList.map((val,key)=>{
          if(value.goods_id == val.goods_id){
            _coun ++;
          }
        })
        if(_coun == 0){this.goodsList.unshift(_obj)}
      });
      this.addConnect = false;
    },
