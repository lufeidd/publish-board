  ...
  <div id="taopuPage" @click="showResult = false">
    ...
    >
      <template slot="footer">
        <span class="click-font" @click="handleCancel" style="margin-right:10px;">取消</span>
            ...
            <div class="choose">
              ...
              >
                ...
                <span
                  ...
                  style="min-width: 40px; min-height: 40px"
                ></span>
                <div class="name" style="width:455px;">{{ gitem.title }}({{ gitem.isbn }})</div>
  ...
  methods: {
      ...
      if (res.code == 0) {
        ...
        this.goodsList = this.goodsList.map((value, key) => {
          if(value.goods_id == res.data[key].goods_id){
            value.result = res.data[key].goods_dms*_nums*this.chooseDay[0];
          }
          return value;
        });
        let _obj = _item,
    ...
    startForcast() {
      if(this.chooseDay.length == 0){
        this.$refs.head.globalTip(1, res.message, 0);
        return;
      }
      if(this.influence.length == 0){
        this.$refs.head.globalTip(1, res.message, res.);
        return
      }
      let ids = [];
