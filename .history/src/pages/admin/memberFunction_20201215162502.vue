                            ...
                            <tr v-for="(item,index) in showList" :key="index">
                              ...
                              <td style="text-align:right;">
                                <span
                                  class="click-font"
                                  v-if="item.click"
                                ><a-switch default-checked v-model="item.status" @change="onChange(item,index,checked)"/></span>
...
export default {
  ...
  methods: {
    ...
    onChange(item,indexchecked){
      console.log(item,checked)
      // this.changeFunction(checked,id);
    },
    confirmChange(type){
      var _this = this;
      this.$confirm({
        title: "确认修改成员权限",
        content: type == 1? '确认将该成员设置为机构管理员吗？':'确认将该成员设置为普通成员吗？',
        okText: "确定",
        cancelText: "取消",
        okType: "primary",
        onOk() {
          _this.changePower(type);
        },
        onCancel() {}
      });
    },
