...
export default {
  ...
  methods: {
    ...
    async getActualData() {
      ...
      if (res.code == 0) {
        ...
        res.data.supplier.yesterday.chart_data.map((value, key) => {
          ...
          _obj.昨日 = value.value;
          this.measureChart1.push(_obj);
        });
        res.data.supplier.today.chart_data.map((value, key) => {
          this.measureChart1[key].今日 = value.value;
        });
        ...
        // console.log(this.measureChart1);
      } else {
        this.isLoading = false;
        if (res.code == 1009) {
    ...
    initActual() {
      let measureChart1Dv = new DataSet.DataView().source(this.measureChart1);
    ...
    initActualLine() {
      ...
      this.measureChart2.data(this.lineData);
      this.measureChart2.scale({
        month: {
          range: [0, 1],
        },
        temperature: {
      ...
      this.measureChart2.axis("temperature", {
        tickLine: null,
        line: null,
        // grid: {
        //   line: {
