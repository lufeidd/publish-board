...
export default {
  ...
  methods: {
    async getData() {
      ...
      if (res.code == 0) {
        ...
        this.list.map((value,key)=>{
          if(value.supplier_id == this.$refs.head.publishInfo.organization_id){
    ...
    onSelect3(item3, index3, _index) {
      ...
      if (this.selectList.includes(item3.supplier_name) && item1.supplier_name !== this.ownName) {
      } else {
