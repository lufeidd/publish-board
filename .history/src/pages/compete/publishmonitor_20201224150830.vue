                        ...
                        <thead>
                          <tr>
                            <td>
                              ...
                              <span
                                class="main-font"
                                :title="index==0?ownName:selectList[index-1]"
                                style="
                                  display: inline-block;
                                  vertical-align: middle;
                                  margin-left: 10px;
                                  ...
                                  width: 310px;
                                "
                                >{{index==0?ownName:selectList[index-1]}}</span
                              >
                            </td>
                            <td style="text-align: right">销售指数</td>
                          </tr>
                    ...
                    <tbody v-if="list.length > 0">
                      <tr v-for="(item, index) in list" :key="index">
                        ...
                        <td>
                          ...
                          <span class="data-tab data-tab-seven" v-if="item.supplier_id == $refs.head.publishInfo.supplier_id">本社</span>
                        </td>
                        <td style="text-align: right">
...
export default {
  data() {
    return {
      ...
      regionFirst:true,
      cateList: [
        // { name: "中国（北京）", type: "首都人口", value: 0.01 },
        // { name: "中国（北京）", type: "城市人口", value: 0.53 },
        // { name: "中国（北京）", type: "农村人口", value: 0.46 },
        // { name: "美国（华盛顿）", type: "首都人口", value: 0.01 },
        // { name: "美国（华盛顿）", type: "城市人口", value: 0.8 },
        // { name: "美国（华盛顿）", type: "农村人口", value: 0.19 },
        // { name: "印度（德里）", type: "首都人口", value: 0.02 },
        // { name: "印度（德里）", type: "城市人口", value: 0.3 },
        // { name: "印度（德里）", type: "农村人口", value: 0.68 },
        // { name: "俄罗斯（莫斯科）", type: "首都人口", value: 0.08 },
        // { name: "俄罗斯（莫斯科）", type: "城市人口", value: 0.66 },
        // { name: "俄罗斯（莫斯科）", type: "农村人口", value: 0.26 },
        // { name: "法国（巴黎）", type: "首都人口", value: 0.16 },
        // { name: "法国（巴黎）", type: "城市人口", value: 0.63 },
        // { name: "法国（巴黎）", type: "农村人口", value: 0.21 },
        // { name: "韩国（首尔）", type: "首都人口", value: 0.19 },
        // { name: "韩国（首尔）", type: "城市人口", value: 0.63 },
        // { name: "韩国（首尔）", type: "农村人口", value: 0.18 },
        // { name: "丹麦（哥本哈根）", type: "首都人口", value: 0.22 },
        // { name: "丹麦（哥本哈根）", type: "城市人口", value: 0.65 },
        // { name: "丹麦（哥本哈根）", type: "农村人口", value: 0.13 },
        // { name: "冰岛（雷克雅未克）", type: "首都人口", value: 0.56 },
        // { name: "冰岛（雷克雅未克）", type: "城市人口", value: 0.38 },
        // { name: "冰岛（雷克雅未克）", type: "农村人口", value: 0.06 },
      ],
    };
  },
  mounted() {
  ...
  methods: {
    ...
    async getCate(){
      ...
      if(this.publishId1){_arr.push(this.publishId1)};
      if(this.publishId2){_arr.push(this.publishId2)};
      if(this.publishId3){_arr.push(this.publishId3)};
      var tStamp = this.$getTimeStamp();
      let data = {
        organization_id: this.$refs.head.publishInfo.organization_id,
      ...
      if(res.code == 0){
        this.pagePower = true;
        this.cateList = [];
        console.log(this.ownName)
        res.data.map((value,key)=>{
          let _obj1 = {},_obj2 = {},_obj3 = {},_obj4 = {};
          _obj1.type = this.ownName;
          _obj1.name = value.category_name;
          _obj1.value = value.details[0].sale_amount;
          this.cateList.push(_obj1);
          if(this.publishId1){
            this.list.map((value,key)=>{
              if(value.supplier_id == this.publishId1){
                _obj2.type = value.supplier_name;
              }
            })
            ...
            this.cateList.push(_obj2)
          }
          if(this.publishId2){
            this.list.map((value,key)=>{
              if(value.supplier_id == this.publishId2){
                _obj3.type = value.supplier_name;
              }
            })
    ...
    showCompete1() {
      ...
      for (let i = 0; i < this.list.length; i++) {
        if (!this.selectList.includes(this.list[i].supplier_name) && this.list[i].supplier_name !== this.ownName) {
    ...
    onSearch1() {
      ...
      this.publishList1 = [];
      this.list.map((value, key) => {
        if (value.supplier_name.indexOf(this.inputVal1) > -1 && value.supplier_name !== this.ownName) {
    ...
    onSearch2() {
      ...
      this.publishList2 = [];
      this.list.map((value, key) => {
        if (value.supplier_name.indexOf(this.inputVal2) > -1 && value.supplier_name !== this.ownName) {
    ...
    onSearch3() {
      ...
      this.publishList3 = [];
      this.list.map((value, key) => {
        if (value.supplier_name.indexOf(this.inputVal3) > -1 && value.supplier_name !== this.ownName) {
    ...
    onSelect1(item1, index1, _index) {
      ...
      if (this.selectList.includes(item1.supplier_name) && item1.supplier_name !== this.ownName) {
      } else {
    ...
    onSelect2(item2, index2, _index) {
      ...
      if (this.selectList.includes(item2.supplier_name) && item2.supplier_name !== this.ownName) {
      } else {
    ...
    onSelect3(item3, index3, _index) {
      ...
      if (this.selectList.includes(item3.supplier_name) && item3.supplier_name !== this.ownName) {
      } else {
    ...
    initStacking1() {
      ...
      this.stackingChart1.data(this.cateList.reverse());
    ...
    initContrast1() {
      ...
      this.contrastChart1.data(this.cateList);
