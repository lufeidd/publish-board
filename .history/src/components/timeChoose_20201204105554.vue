...
export default {
  ...
  methods:{
    ...
    yearChange(e) {
      console.log(e._d.getFullYear().toString())
      if (
    ...
    disabledEndMonth(endValue) {
      // const startValue = new Date(this.$currenTime);
      const startValue = this.$moment(this.$currenTime);
      // console.log(endValue.month(),startValue.month());
      if (!endValue || !startValue) {
        return false;
      }
      if(startValue.year() >= endValue.year()){
        return startValue.month() <= endValue.month();
      }
    },
    disabledEndDate(endValue) {
      ...
      return startValue.year() <= endValue.valueOf();
    },
  }
};
</script>
